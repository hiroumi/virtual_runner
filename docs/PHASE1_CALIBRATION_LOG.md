...
（`fullscreen` は当初 `false` で保存されていたが、実プレイ時はフルスクリーン運用が前提とのユーザー確認により `true` に変更・再コミット。）
...
## 2026-09-02: fullscreen設定の確定

`fullscreen` を `false`（ウィンドウ表示）から `true` に変更。実際にアクセサリを覗いてプレイする運用ではフルスクリーンが前提であるとユーザーが確認したため。
